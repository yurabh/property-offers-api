...
Репозиторій: _<посилання додати>_
...
Обробка імпорту виконується у джобі, тож воркер має бути запущений:
...
Тести ганяються на **MySQL** (схема `property_offers_test`, створюється автоматично з
...
```json
{ "client_reference": "web-order-9f782b1c", "customer_name": "John Smith", "customer_email": "john@example.com" }
```
...
загортають результат у ресурс.

| Дія | Хто викликає |
|---|---|
| `Actions\Imports\RegisterImport` | `POST /api/imports` — фіксує імпорт і ставить джобу в чергу |
| `Actions\Imports\ProcessImport` | `ProcessImportJob` — розбирає пропозиції |
| `Actions\Properties\SearchProperties` | `GET /api/properties` |
| `Actions\Reservations\ReserveOffer` | `POST /api/offers/{offer}/reservations` |
...
`Propertie` будує один запит: підзапит ранжує пропозиції
...
Розглядався варіант без віконної функції — анти-join `LEFT JOIN offers cheaper ... WHERE cheaper.id IS NULL`.
Він не потребує `selectRaw`, але дублює всі умови актуальності у двох місцях і на
обсягах програє. Обрано віконну функцію.
...
і `completed_at`. Лічильник `processed_offers` оновлюється по ходу, тож прогрес
видно через `GET /api/imports/{id}`.
...
DB::transaction(function () use ($offer, $data) {
    ...
    // перевірка expires_at і available_units
...
Другий рівень — **`unique(client_reference)`**: якщо клієнт ретраїть той самий запит
(мережевий таймаут, повторний клік), другого бронювання не з'явиться навіть при
повній одночасності — унікальний ключ відсіче дубль, і у відповідь повернеться вже
створене бронювання з кодом `200`.
...
| `ProcessImportJobTest` | створення об'єктів і пропозицій, оновлення пропозиції з іншого імпорту, захист від імпортів не по порядку, повторний запуск джоби, статус `failed` |
...
- **«Дати збігаються»** трактується як точний збіг `check_in` / `check_out` пропозиції
  з параметрами пошуку: постачальник надсилає готові пропозиції на конкретний
  проміжок, а не безперервний календар доступності.
- Сирі пропозиції імпорту зберігаються в JSON-колонці `imports.payload`. Для
  промислових обсягів це стала б окрема таблиця рядків імпорту, тут навмисно простіше.
- Бронювання лише зменшує `available_units`; скасування бронювань виходить за межі ТЗ.
- Аутентифікація та rate limiting не реалізовані — у завданні їх немає.
